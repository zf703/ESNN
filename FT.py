...
from preprocess.lib_extension import Substract_average_plus_P_2, IFFT, Smooth_Gaussian, Center_surround_diff, Normalise, RGB_0_255, Concatenation
...
eeg_type_data = collections.namedtuple('eeg_type_data', ['patient_id', 'label', 'data', 's1', 's2', 'd'])
def convert_to_fft(window_length, overlap, sampling_frequency, file_path, label):
    ...
    named_data = eeg_type_data(patient_id=id, label=label, data=fft_data, s1=[], s2=[], d=[])
...
def create_s1(file_data):
    ...
    named_data = eeg_type_data(patient_id=file_data.patient_id, label=file_data.label, data=file_data.data, s1=s1_data, s2=[], d=[])
...
def create_s2(file_data):
    ...
    named_data = eeg_type_data(patient_id=file_data.patient_id, label=file_data.label, data=file_data.data, s1=file_data.s1, s2=s2_data, d=[])
...
def create_d(file_data):
    ...
    for i in range(0, l):
        ...
        d_norm = pipeline2.apply1(window_ft_norm, window_s1_norm, window_s2_norm)
    ...
    named_data = eeg_type_data(patient_id=file_data.patient_id, label=file_data.label, data=file_data.data, s1=file_data.s1, s2=file_data.s2, d=d_data)
...
def main():
    ...
    path = 'D:\dataset_19'
    hc_save_d_path = 'D:\D\HC'
    mdd_save_d_path = 'D:\D\MDD'
    hc_path = path + '\\' + 'HC'
    mdd_path = path + '\\' + 'MDD'
    ...
    for window_length in window_lengths:
        ...
        for overlap in overlaps:
            print(overlap)
            save_d_data_dir = os.path.join(hc_save_d_path, 'd_HC_' + 'wl_' + str(window_length) + '_o_' + str(overlap))
            ...
            for file_path in sorted(hc_list_path):
                converted_fft_data = convert_to_fft(window_length, overlap, sampling_frequency, file_path, 0)
                ...
                converted_d_data = create_d(converted_s2_data)
    ...
    for window_length in window_lengths:
        ...
        for overlap in overlaps:
            print(overlap)
            save_d_data_dir = os.path.join(mdd_save_d_path, 'd_MDD_' + 'wl_' + str(window_length) + '_o_' + str(overlap))
            ...
            for file_path in sorted(mdd_list_path):
                converted_fft_data = convert_to_fft(window_length, overlap, sampling_frequency, file_path, 1)
                base_dir = file_path.split('\\')[3].split('.')[0] + '.pkl'
                converted_s1_data = create_s1(converted_fft_data)
                converted_s2_data = create_s2(converted_s1_data)
                converted_d_data = create_d(converted_s2_data)
